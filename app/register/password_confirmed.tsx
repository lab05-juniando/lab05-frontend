...
// ---------- NOME ----------
type NameFieldProps = { onValidityChange: (isValid: boolean, name: string) => void };
...
export function NameField({ onValidityChange }: NameFieldProps) {
    ...
    return (
        <div>
            ...
            />
            {touched && (
                <span className={`text-xs mt-1 block ${isValid ? "text-green-500" : "text-red-500"}`}>
                    {isValid ? "Nome válido" : "Informe nome e sobrenome (mín. 3 letras cada)"}
                </span>
            )}
        </div>
    );
...
// ---------- SENHA + CONFIRMAR ----------
type PasswordFieldsProps = { onValidityChange: (isValid: boolean, pwd: string) => void };
    ...
    return (
        <>
            <div>
                ...
                />
                {pwdTouched && (
                    <span className={`text-xs mt-1 block ${isStrong ? "text-green-500" : "text-red-500"}`}>
                        {isStrong ? "Senha forte" : "Senha fraca — use 8+ caracteres, maiúscula, número e símbolo"}
                    </span>
                )}
            </div>
            ...
            <div>
                ...
                />
                {confirmTouched && (
                    <span className={`text-xs mt-1 block ${pwdsMatch ? "text-green-500" : "text-red-500"}`}>
...
// ---------- CNPJ ----------
type CnpjFieldProps = { onValidityChange: (isValid: boolean, cnpj: string) => void };
